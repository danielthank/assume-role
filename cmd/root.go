...
func init() {
	rootCmd.PersistentFlags().DurationVarP(&duration, "duration", "d", time.Hour, "The duration that the credentials will be valid for")
	rootCmd.PersistentFlags().StringVarP(&role, "role", "r", "", "Role to be switched to")
	rootCmd.PersistentFlags().StringVarP(&outputFormat, "output", "o", defaultFormat(), "Output format")
}
...
func runRoot() error {
	...
	if roleArnRe.MatchString(role) {
		...
		if !ok {
			return fmt.Errorf("%s not in %s", role, configFilePath)
		}
	...
	default:
		return fmt.Errorf("outputFormat should be powershell, bash, or fish")
	}
	return nil
}
